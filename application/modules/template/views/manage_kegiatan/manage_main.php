...
<head>
  <!-- Title -->
  <title><?= ($this->uri->segment(1) ? ucwords(str_replace('-', ' ', $this->uri->segment(1)).' '.$this->session->userdata('manage_namaKegiatan').' - '.$WEB_JUDUL) : $WEB_JUDUL);?></title>
...
<body class="footer-offset">
  ...
  <main id="content" role="main" class="main pointer-kegiatan">
    <!-- Content -->
    ...
    <ul class="step step-icon-sm step-avatar-sm">
      <?php if ($aktivitas == FALSE): ?>
        ...
        <?php if ($c_aktivitas > 8): ?>
          <a class="btn btn-block btn-white" href="<?= site_url('manage-kegiatan/aktivitas-kegiatan') ?>">Lihat semua <i class="tio-chevron-right"></i></a>
        <?php endif; ?>
      </div>
      <!-- End Body -->
    </div>
